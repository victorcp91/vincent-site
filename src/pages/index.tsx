      ...
      <main className="px-10 pb-2 w-full max-w-2xl m-auto">
        ...
        <section className="flex my-3">
          ...
          <div className="flex flex-col">
            {!!home.attributes.email && (
              <a className="mb-1" href={`mailto:${home.attributes.email}`}>
                {home.attributes.email}
              </a>
            )}
            {!!home.attributes.phone && (
              <a className="mb-1" href={home.attributes.phone_link}>
                {home.attributes.phone}
              </a>
            )}
            {!!home.attributes.address && (
              <a
                className="mb-1"
                href={home.attributes.address_link}
                target="_blank"
              >
                {home.attributes.address}
              </a>
...
export async function getServerSideProps({
  ...
  try {
    const [main, home] = await Promise.all([
      ...
      fetch(`${process.env.API_PATH}/home?populate=*`).then((home) =>
        home.json()
      ),
    ...
    if (home?.attributes?.profile_picture?.data) {
      home.attributes.profile_picture =
        home.attributes.profile_picture.data.attributes;
    }
